...
var AccountUser = require('./objects/AccountUser');
...
var EmailSendDefinition = require('./objects/EmailSendDefinition');
var Folder = require('./objects/Folder');
var ImportDefinition = require('./objects/ImportDefinition');
var ImportResultsSummary = require('./objects/ImportResultsSummary');
...
var ET_Client = function(clientId, clientSecret, stack, {origin, authOrigin, soapOrigin, authOptions = {}, globalReqOptions = {}} = {}) {
    ...
    if (!clientId) {
        throw new Error('The client clientId is required.');
    }

    if (!clientSecret && (authOptions.applicationType != 'public')) {
        throw new Error('The client clientSecret is required.');
    }
    ...
    if(authOptions && authOptions.authVersion === 2){
        ...
        if(authOptions.applicationType == 'public' || authOptions.applicationType == 'web'){
            if(!authOptions.redirectURI || !authOptions.authorizationCode){
                throw new Error('The client redirectURI and authorizationCode are required for Public/Web App Flow.');
            }
        }
    }
...
ET_Client.prototype.initSoap = function(auth, origin, stack, soapOrigin, globalReqOptions) {
    ...
    client.origin = origin;
    // OVERRIDE METHODS
    ['create', 'retrieve', 'update', 'delete', 'perform'].forEach(method => { // for each method
...
//								Object Factory
//****************************************************************************************
ET_Client.prototype.account = function(options) {
	return new Account(this, options);
};

ET_Client.prototype.accountUser = function(options) {
	return new AccountUser(this, options);
};
...
ET_Client.prototype.emailSendDefinition = function(options) {
	return new EmailSendDefinition(this, options);
};
...
ET_Client.prototype.importDefinition = function(options) {
	return new ImportDefinition(this, options);
};

ET_Client.prototype.importResultsSummary = function(options) {
	return new ImportResultsSummary(this, options);
};
